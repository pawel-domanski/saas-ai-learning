...
export default async function AiOpDocumentDetail({ params }: { params: { id: string; documentid: string } }) {
  ...
  let isRead = false;
  if (user) {
    const progress = await getUserGuideProgress(user.id, id, parseInt(documentid, 10));
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <DocumentCompleteButton guideId={id} documentId={parseInt(documentid, 10)} initialCompleted={isRead} />
      </div>
