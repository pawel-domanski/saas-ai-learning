...
import { getUser, getUserAiOpProgress } from '@/lib/db/queries';

export default async function AiOpDocumentDetail({ params }: { params: Promise<{ id: string; documentid: string }> }) {
  const { id, documentid } = await params;
  const filePath = path.join(process.cwd(), 'aiop.json');
  ...
  // Determine if this document is already marked read
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <Link href={`/ai-op/${id}`} className="inline-flex items-center text-blue-600 hover:underline">
