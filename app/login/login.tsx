...
export function Login({ mode = 'signin' }: { mode?: 'signin' | 'signup' }) {
  ...
  const [birthCity, setBirthCity] = useState(state.birthCity || '');
  const [birthCountry, setBirthCountry] = useState(state.birthCountry || '');

  // Update state when form state changes
  useEffect(() => {
    if (state.birthCity) setBirthCity(state.birthCity);
    if (state.birthCountry) setBirthCountry(state.birthCountry);
  }, [state.birthCity, state.birthCountry]);
